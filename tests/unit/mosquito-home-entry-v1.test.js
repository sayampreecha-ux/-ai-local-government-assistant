...
test('GP008 exposes mosquito one-page as a permanent public-health task without duplicating the temp-staff planner', async () => {
  ...
  assert.doesNotMatch(gp008, /id=["']tempStaffMaintenanceFundEntry["']/i);
  ...
  assert.equal(staticTaskCount, 16, 'GP008 must contain exactly 16 static task entries after temp-staff deduplication');
