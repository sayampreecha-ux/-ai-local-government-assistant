export const BUDGET_OFFICIAL_DOCUMENT_PARSER_VERSION = '1.0';
...
const LABELS = Object.freeze({
  ...
  expense: [
    ['central-budget', /งบกลาง/i],
    ['personnel', /งบบุคลากร/i],
    ['operations', /งบดำเนินงาน/i],
    ['investment', /งบลงทุน/i],
    ['subsidy', /งบเงินอุดหนุน|เงินอุดหนุน(?!ทั่วไป)/i],
    ['other-expense', /รายจ่ายอื่น/i]
  ]
});

function linesOf(rawContent) {
  return text(rawContent).split('\n').map(normalizeSpace).filter(Boolean);
}
...
function extractLabelRows(lines, definitions) {
  ...
  for (const [key, pattern] of definitions) {
    ...
    for (const line of candidates) {
      ...
      if (!best || amount > best.amount) best = { key, label: line.slice(0, 180), amount, status: 'verified' };
    }
...
function findBudgetTotal(lines, revenueRows, expenseRows) {
  ...
  for (const line of lines) {
    if (!/(รวมรายจ่าย|รายจ่ายทั้งสิ้น|งบประมาณรายจ่าย|รวมประมาณการรายรับ|ประมาณการรายรับรวม|รวมรายรับ)/i.test(line)) continue;
    const numbers = moneyNumbers(line).filter(number => number >= 100000);
    explicit.push(...numbers);
  }
...
export function parseBaselineBudgetDocument(rawContent, { targetYear = null } = {}) {
  ...
  const revenueItems = extractLabelRows(lines, LABELS.revenue);
  const expenseItems = extractLabelRows(lines, LABELS.expense);
  const fiscalYear = fiscalYearFrom(lines, targetYear ? Number(targetYear) - 1 : null);
  const total = findBudgetTotal(lines, revenueItems, expenseItems);
  const errors = [];
  if (!fiscalYear) errors.push('fiscalYear:not-found');
  if (!Number.isFinite(total)) errors.push('total:not-found');
  if (expenseItems.length < 2 && revenueItems.length < 2) errors.push('budget-breakdown:insufficient');
...
function cleanProjectName(line) {
  const cells = line.includes('|') ? line.split('|').map(normalizeSpace).filter(Boolean) : [line];
  const candidates = cells.filter(cell => projectCandidate(cell));
...
export function parseTargetYearPlanDocument(rawContent, { targetYear = null } = {}) {
  const lines = linesOf(rawContent);
  const detectedYear = fiscalYearFrom(lines, targetYear);
  const projectLines = lines.filter(projectCandidate);
  const names = unique(projectLines.map(cleanProjectName)).slice(0, 1000);
