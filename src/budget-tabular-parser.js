...
export const BUDGET_TABULAR_PARSER_VERSION = '1.2';
...
export function parseCsvText(csvText = '') {
  ...
  const rows = lines.slice(1).map(line => { const values = parseCsvLine(line); return Object.freeze(Object.fromEntries(headers.map((header, index) => [header, text(values[index] ?? '')]))); });
  return Object.freeze({ headers: Object.freeze(headers), rows: Object.freeze(rows) });
}
function findHeader(headers, candidates, { exactFirst = true, exclude = [] } = {}) {
  const normalized = headers.map(header => [header, normalizeHeader(header)]).filter(([, header]) => !exclude.some(item => header.includes(normalizeHeader(item))));
  if (exactFirst) {
    const exact = normalized.find(([, header]) => candidates.some(candidate => header === normalizeHeader(candidate)));
...
export function extractBudgetFromRows({ purpose, headers = [], rows = [] } = {}) {
  ...
  const amountHeader = findHeader(headers, ['จำนวนเงิน', 'ยอดเงิน', 'amount', 'total', 'งบประมาณ'], { exclude: ['ปีงบประมาณ', 'fiscalyear'] });
  ...
  if (purpose === 'baselineBudget') {
    ...
    if (!year) warnings.push('fiscalYear:not-found'); if (total == null) warnings.push('total:not-found');
  ...
  if (purpose === 'personnelObligations') {
    const items = rows.map((row, index) => Object.freeze({ key: `personnel-${index + 1}`, label: labelHeader ? text(row[labelHeader]) : `รายการ ${index + 1}`, amount: amountHeader ? number(row[amountHeader]) : null, status: 'pending-confirmation' })).filter(item => item.label || item.amount != null);
    const total = sumItems(items); if (!items.length) warnings.push('personnel:items-not-found');
  ...
  if (purpose === 'budgetTotals') {
    const revenueItems = []; const expenseItems = [];
    rows.forEach((row, index) => {
      const type = typeHeader ? normalizeHeader(row[typeHeader]) : '';
      const item = Object.freeze({ key: `row-${index + 1}`, label: labelHeader ? text(row[labelHeader]) : `รายการ ${index + 1}`, amount: amountHeader ? number(row[amountHeader]) : null, status: 'pending-confirmation' });
      if (/รายรับ|revenue|income/.test(type)) revenueItems.push(item); else if (/รายจ่าย|expense|expenditure/.test(type)) expenseItems.push(item);
    });
    if (!typeHeader) warnings.push('budgetTotals:type-column-not-found'); if (!revenueItems.length) warnings.push('budgetTotals:revenue-not-found'); if (!expenseItems.length) warnings.push('budgetTotals:expense-not-found');
    return Object.freeze({ parsed: Object.freeze({ revenueTotal: sumItems(revenueItems), expenseTotal: sumItems(expenseItems), revenueItems: Object.freeze(revenueItems), expenseItems: Object.freeze(expenseItems) }), warnings: Object.freeze(warnings) });
  }
...
export async function parseBudgetCsvFile(file, intake) {
  ...
  const review = createParsedBudgetReview({ fileRef: intake.file.safeRef, contentHash: intake.file.contentHash, purpose: intake.purpose, parsed: extraction.parsed, parser: `budget-tabular-parser/${BUDGET_TABULAR_PARSER_VERSION}` });
  return Object.freeze({ status: review.status, failClosed: true, parserVersion: BUDGET_TABULAR_PARSER_VERSION, review, warnings: Object.freeze([...extraction.warnings, ...review.warnings]), governance: Object.freeze({ rawFileContentReturned: false, parserOutputIsNotEvidence: true, humanConfirmationRequired: true }) });
}
export function parseBudgetWorkbookRows({ purpose, rows = [] } = {}) {
  ...
  const headers = Object.keys(rows[0] || {}); return extractBudgetFromRows({ purpose, headers, rows });
}
