...
for(let i=1;i<=13;i++){const file=`gp${String(i).padStart(3,'0')}.html`;const html=await readFile(file,'utf8');for(const asset of assets){const rel=asset.replace('assets/','assets/');if(!html.includes(rel))throw new Error(`${file}: missing ${rel}`)}if(!/(?:สร้าง Prompt|สร้างคำสั่ง)/.test(html))throw new Error(`${file}: missing generator`)}
