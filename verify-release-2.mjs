...
const pages=['index.html',...Array.from({length:13},(_,i)=>`gp${String(i+1).padStart(3,'0')}.html`),'privacy.html','terms.html'];
...
if(!index.includes('id="chatForm"'))throw new Error('index: missing primary chat form');
if(!index.includes('official-search-connector.js'))throw new Error('index: missing official search connector');
if(!index.includes('privacy-guard.js'))throw new Error('index: missing privacy guard');
console.log(`GovPrompt Thailand Release 2.0 validated: ${pages.length} pages, 13 assistants retained behind lean home UI, ${assets.length} shared assets`);
