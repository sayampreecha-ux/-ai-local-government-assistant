...
const registry=await readFile('assets/js/core/prompt-registry.js','utf8');for(let i=1;i<=13;i++){const moduleId=`GP${String(i).padStart(3,'0')}`;const page=`${moduleId.toLowerCase()}.html`;if(!registry.includes(`'${moduleId}'`)||!registry.includes('path: `${moduleId.toLowerCase()}.html`'))throw new Error(`prompt registry: missing route for ${moduleId} (${page})`)}
