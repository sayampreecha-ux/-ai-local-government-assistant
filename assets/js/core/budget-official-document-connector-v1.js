...
export const BUDGET_OFFICIAL_DOCUMENT_CONNECTOR_VERSION = '1.0';

const safe = (value, max = 1800) => String(value ?? '').trim().slice(0, max);

export async function readOfficialBudgetDocument(sourceUrl, targetKey, { targetYear = null, endpoint = '/api/official-document' } = {}) {
  ...
  if (!response.ok || payload?.ok !== true || !safe(payload?.rawContent, 1_000_000)) {
    return Object.freeze({
      status: 'blocked-document-reader',
      readDocument: null,
      errors: Object.freeze([safe(payload?.error || `http-${response.status}`, 160)])
    });
  }
  const parsed = parseOfficialBudgetDocument(targetKey, payload.rawContent, { targetYear });
  if (!parsed.valid) {
    return Object.freeze({ status: 'blocked-document-parse', readDocument: null, errors: parsed.errors });
  }
  return Object.freeze({
    status: 'ready',
    readDocument: Object.freeze({
      sourceUrl: url,
      resolvedUrl: safe(payload.resolvedUrl),
      contentHash: safe(payload.contentHash, 128),
      readAt: safe(payload.extractedAt, 80),
      ...
      extraction: Object.freeze({
        provider: safe(payload.provider, 80),
        requestId: safe(payload.requestId, 160),
...
export function createBudgetOfficialDocumentConnector(options = {}) {
  return Object.freeze({
    read: (sourceUrl, targetKey, context = {}) => readOfficialBudgetDocument(sourceUrl, targetKey, { ...options, ...context })
  });
}

export default Object.freeze({ version: BUDGET_OFFICIAL_DOCUMENT_CONNECTOR_VERSION, readOfficialBudgetDocument, createBudgetOfficialDocumentConnector });
