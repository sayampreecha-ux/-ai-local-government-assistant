(() => {
  ...
  const TOR_FIRST_SUCCESS_COPY = Object.freeze({
    ...
    intro: 'ไม่ต้องเขียน Prompt และยังไม่ต้องรู้สเปกทั้งหมด ตอบ 3 เรื่องสำคัญเท่าที่มี แล้ว GP จะถามต่อเฉพาะสิ่งที่จำเป็น',
    note: 'จากนั้น GP จะช่วยจัดโครง TOR ตรวจลักษณะงาน ความเสี่ยงการล็อกสเปก/การแข่งขัน และหลักฐานที่ยังต้องยืนยัน โดยผู้ใช้ตรวจและอนุมัติก่อนใช้จริง',
  ...
  function inferIntent(text, route = {}) {
    ...
    if (/(?:ประชาสัมพันธ์|วิดีโอ|คำกล่าว|สคริปต์|โพสต์|อินโฟกราฟิก)/i.test(value)) return 'pr';
  ...
  function fieldSatisfied(key, text) {
    ...
    switch (key) {
      ...
      case 'target': return /(?:กลุ่มเป้าหมาย|ผู้เข้าร่วม|ประชาชน|ผู้สูงอายุ|เด็ก|เยาวชน|นักเรียน|ข้าราชการ|เจ้าหน้าที่|บุคลากร|ชุมชน|อสม\.?|รพ\.?สต\.?|facebook|เฟซบุ๊ก|โซเชียล|คณะกรรมการ|ผู้บริหาร|\d+\s*คน)/i.test(value);
      case 'duration': return /(?:\d+(?:\.\d+)?\s*(?:วินาที|วิ|นาที)|ให้\s*gp\s*แนะนำ|ให้ระบบแนะนำ|ไม่แน่ใจ|ไม่ทราบ)/i.test(value);
