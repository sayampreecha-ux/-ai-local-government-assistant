(() => {
  ...
  function createQualityGuidance(text) {
    ...
    if (isPrCreation) {
      return Object.freeze([
        'ตอบแบบ Answer First: จัดทำชิ้นงานประชาสัมพันธ์พร้อมใช้ก่อน ไม่อธิบายกฎงานราชการที่ไม่เกี่ยวข้อง',
    ...
    if (/(?:เบิก|เบิกจ่าย|ค่าเดินทาง|ค่าแท็กซี่|เงินบำรุง|เงินสะสม|เงินสำรองจ่าย|ใช้เงิน|งบประมาณ)/i.test(text)) {
      guidance.push('ถ้าเป็นการเงิน/เบิกจ่าย ให้ตอบให้ชัดก่อนว่า “เบิกได้ / เบิกไม่ได้ / มีเงื่อนไข / ยังยืนยันไม่ได้” แล้วสรุปฐานอำนาจ เงื่อนไข เอกสารประกอบ และจุดที่ต้องอนุมัติหรือใช้ดุลพินิจ');
    }
  ...
  function createToolRoutingPlan({ question, attachments = [] } = {}) {
    ...
    if (needsCurrentWeb || needsPrimarySource) {
      ...
      instructions.push('ให้ AI ที่กำลังตอบใช้ความสามารถค้นเว็บสดของบัญชีหรือแพลตฟอร์มของผู้ใช้เองเมื่อจำเป็น โดยยึดแหล่งราชการ/ต้นฉบับก่อน ตรวจฉบับที่ใช้กับบุคคลและช่วงเวลาเกิดเหตุ พร้อมฉบับปัจจุบันประกอบ ระบุหน่วยงานผู้ออก วันที่ และแหล่งอ้างอิง และห้ามฟันธงจากแหล่งสรุปหรือถือว่าฉบับใหม่ที่สุดใช้กับทุกเหตุการณ์');
      instructions.push('การค้นสดให้ใช้เครื่องมือค้นเว็บของ AI ฝั่งผู้ใช้เป็นค่าเริ่มต้น บริการค้นของ GovPrompt ใช้เฉพาะเมื่อผู้ใช้เลือกใช้หรือ AI ฝั่งผู้ใช้ไม่มีความสามารถค้นเว็บ');
    }
  ...
  function formatToolRoutingInstructions(plan) {
    ...
    return [
      ...
      '- หากเครื่องมือที่แนะนำไม่มีในสภาพแวดล้อมของ AI ฝั่งผู้ใช้ ให้บอกตรง ๆ ว่ายังยืนยันข้อมูลสดไม่ได้ และทำเฉพาะส่วนที่ทำได้ ห้ามอ้างว่าได้ค้นหรือเปิดข้อมูลแล้ว'
    ].join('\n');
  }
