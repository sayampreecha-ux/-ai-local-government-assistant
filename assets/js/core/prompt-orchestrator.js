(() => {
  ...
  const HIGH_RISK_TERMS = Object.freeze([
    ...
    'เงินเดือน', 'โบนัส', 'ประโยชน์ตอบแทน', 'บรรจุ', 'แต่งตั้ง', 'เลื่อน', 'สิทธิ'
  ]);

  const MEDIUM_RISK_TERMS = Object.freeze([
    'ขั้นตอน', 'แนวทาง', 'หนังสือราชการ', 'บันทึกข้อความ', 'โครงการ', 'สภา', 'ประชุม'
  ]);

  const DECISION_TERMS = /(?:ได้ไหม|ได้หรือไม่|มีสิทธิ|ไม่มีสิทธิ|เบิกได้|เบิกไม่ได้|ทำได้|ทำไม่ได้|มีอำนาจ|อนุมัติได้|จ่ายได้|ชอบด้วย|ถูกกฎหมาย|ผิดกฎหมาย|ต้องทำไหม|ต้องทำหรือไม่)/i;
  const MULTI_CONDITION_TERMS = /(?:สิทธิ|คุณสมบัติ|เงื่อนไข|โบนัส|ประโยชน์ตอบแทน|เบิก|จัดซื้อ|จัดจ้าง|tor|บรรจุ|แต่งตั้ง|เลื่อนเงินเดือน|เงินเดือน|วินัย|อำนาจ|อนุมัติ|สั่งจ่าย)/i;
  ...
  window.GovPromptCore.PROMPT_STANDARD_VERSION = '7.1';
