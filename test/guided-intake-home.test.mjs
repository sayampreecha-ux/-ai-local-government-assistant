...
test('home loads guided intake directly before bridge and home handler with fresh cache keys', () => {
  const intake = homeHtml.match(/assets\/js\/core\/guided-intake-v1\.js\?v=[^"']+/)?.[0];
  const bridge = homeHtml.match(/assets\/js\/ui\/quick-action-guided-bridge-v1\.js\?v=[^"']+/)?.[0];
  const home = homeHtml.match(/assets\/js\/home-v3\.js\?v=[^"']+/)?.[0];
  const feedback = homeHtml.match(/assets\/js\/ui\/pilot-feedback-ui\.js\?v=[^"']+/)?.[0];
  assert.ok(intake);
  assert.ok(bridge);
  assert.ok(home);
  assert.ok(feedback);
  assert.match(intake, /\?v=\d+\.\d+\.\d+$/);
  assert.match(bridge, /\?v=\d+\.\d+\.\d+$/);
  assert.match(home, /\?v=\d+\.\d+\.\d+$/);
  assert.match(feedback, /\?v=\d+\.\d+\.\d+$/);
