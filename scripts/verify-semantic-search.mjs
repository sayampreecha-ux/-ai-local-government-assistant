...
const semanticScript = '<script src="assets/js/core/semantic-search.js"></script>';
for (let number = 1; number <= 12; number += 1) {
  const file = `gp${String(number).padStart(3, '0')}.html`;
  const current = (await readFile(file, 'utf8')).replace(/\r\n/g, '\n');
  const baseline = execFileSync('git', ['show', `7102602:${file}`], { encoding: 'utf8' }).replace(/\r\n/g, '\n');
...
console.log('Semantic Knowledge Search verification passed with GP001-GP012 unchanged.');
